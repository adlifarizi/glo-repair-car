...
@section('title', isset($pemasukan) ? 'Ubah Pemasukan' : 'Tambah Pemasukan')
        ...
        <div class="p-4">
            ...
            <div class="flex items-center md:items-center gap-4 justify-start my-4">
                ...
                <h2 class="font-medium text-2xl md:text-lg text-red-800 md:text-gray-400 capitalize md:normal-case">
                    {{ isset($pemasukan) ? 'Ubah Pemasukan' : 'Tambah Pemasukan' }}
                </h2>
            </div>

            <!-- Form -->
            <div class="p-4 bg-white rounded-xl">
                <p class="text-lg font-semibold">{{ isset($pemasukan) ? 'Ubah Pemasukan' : 'Tambah Pemasukan' }}</p>
                <p class="text-gray-600">
                    {{ isset($pemasukan) ? 'Ubah pemasukan yang dirasa tidak sesuai' : 'Tambah pemasukan secara manual' }}
                </p>

                <form method="POST" class="w-full mt-6"
                    action="{{ isset($pemasukan) ? route('entry.update', $pemasukan->id) : route('entry.store') }}">
                    @csrf
                    @if(isset($pemasukan))
                        @method('PUT')
                    @endif
                    ...
                    <div class="grid md:grid-cols-[130px_1fr] grid-cols-1 md:items-center mb-4 gap-2 md:gap-4">
                        ...
                        <input type="number" name="id_servis" id="id_servis"
                            value="{{ old('id_servis', $pemasukan->id_servis ?? '') }}" placeholder="*jika tidak ada kosongkan"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 md:gap-4">
                        <div class="grid md:grid-cols-[130px_1fr] grid-cols-1 md:items-center mb-4 gap-2 md:gap-4">
                            ...
                            <div class="flex">
                                ...
                                <input type="number" name="nominal" id="nominal"
                                    value="{{ old('nominal', $pemasukan->nominal ?? '') }}" placeholder="nominal pemasukan"
                        ...
                        <div class="grid md:grid-cols-[auto_1fr] grid-cols-1 md:items-center mb-4 gap-2 md:gap-4">
                            <label for="tanggal_pemasukan" class="text-sm font-medium text-gray-700">Tanggal Pemasukan</label>
                            <div class="flex items-center justify-between border border-gray-300 rounded-md px-4 py-2"
                                ...
                                <input type="text" name="tanggal_pemasukan" id="tanggal_pemasukan"
                                    value="{{ old('tanggal_pemasukan', $pemasukan->tanggal_pemasukan ?? '') }}"
                    ...
                    <div class="grid md:grid-cols-[130px_1fr] grid-cols-1 md:items-center mb-4 gap-2 md:gap-4">
                        ...
                        <div class="flex items-center justify-between border border-gray-300 rounded-md px-4 py-2 relative">
                            ...
                            <input type="file" name="bukti_pemasukan" id="bukti_pemasukan" accept="image/*"
                                class="absolute inset-0 opacity-0 cursor-pointer" />

                            <!-- Text input untuk menunjukkan nama file terpilih -->
                            <span id="file-name" class="w-full truncate text-gray-700">
                                {{ old('bukti_pemasukan', $pemasukan->bukti_pemasukan ?? '') }}
                            </span>
                    ...
                    <div class="grid md:grid-cols-[130px_1fr] grid-cols-1 md:items-start mb-4 gap-2 md:gap-4">
                        ...
                        <textarea name="keterangan" id="keterangan" rows="4" placeholder="keterangan"
                            class="w-full border border-gray-300 rounded-md px-4 py-2 focus:ring-gray-500 focus:ring-1 ring-inset focus:outline-none">{{ old('keterangan', $pemasukan->keterangan ?? '') }}</textarea>
                    </div>
